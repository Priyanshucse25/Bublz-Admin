  ...
  <div class="min-h-screen bg-gray-50 p-4 sm:p-0 lg:p-0">
    <div class="mx-auto max-w-7xl">
      ...
      <div class="space-y-4 bg-white p-6 shadow-md">
        ...
        <div v-else-if="faqs.length === 0" class="py-12 text-center text-gray-500">No FAQs have been added yet.</div>
        <div v-else v-for="faq in faqs" :key="faq.id" class="rounded-lg border border-gray-200">
          <div class="flex w-full items-center justify-between p-4 text-left font-semibold text-gray-800">
            ...
            <div class="flex items-center space-x-4">
              <button
                @click="handleToggleStatus(faq)"
                :class="faq.status === 'active' ? 'bg-green-500' : 'bg-gray-300'"
                class="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full  transition"
                title="Toggle Status"
              >
                <span
              ...
              <button @click="handleDeleteFaq(faq.id)" class="text-red-500 hover:text-red-700" title="Delete">
                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          ...
          <Transition name="collapse">
            <div v-if="openFaqId === faq.id" class="px-4 pb-4">
              <div
                ...
                :key="index"
                class="prose prose-sm max-w-none border-t pt-2 mt-2 text-gray-600"
              >
                <p>{{ answer }}</p>
              </div>
            </div>
          ...
          <form @submit.prevent="handleSubmit" class="flex h-[90vh] flex-col">
            ...
            <div class="flex-grow space-y-6 overflow-y-auto p-6">
              ...
              <div class="flex items-center justify-between">
                ...
                <button
                  ...
                  class="relative inline-flex h-6 w-11 cursor-pointer rounded-full border-2 transition"
                >
                  <span
            ...
            <div class="flex justify-end border-t p-4">
              <button
                ...
                class="rounded-lg bg-green-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-green-700 disabled:opacity-50"
              >
                {{ isPosting || isSubmitting ? 'Submitting...' : (isEditing ? 'Update FAQ' : 'Submit FAQ') }}
              </button>
            </div>
          </form>
        </div>
...
const {
  faqs, isLoading, isSubmitting, isPosting, error, submitError
} = storeToRefs(faqStore);
const { getFaqs, createFaq, updateFaq, deleteFaq } = faqStore;
...
const handleSubmit = async () => {
  if (isEditing.value) {
    await updateFaq(newFaq.value.id, { ...newFaq.value });
    if (!submitError.value) closeModal();
  } else {
...
const handleToggleStatus = async (faq) => {
  ...
  await updateFaq(faq.id, updatedFaq);
};
</script>
