  ...
  <div class="min-h-screen bg-gray-50 p-4 sm:p-6 lg:p-0">
    <div class="mx-auto max-w-7xl">
      ...
      <div class="space-y-4 bg-white p-6 shadow-md">
        <div v-if="faqs.length === 0" class="py-12 text-center text-gray-500">
            No FAQs have been added yet.
        </div>
        <div v-for="faq in faqs" :key="faq.id" class="rounded-lg border border-gray-200">
        ...
        <div class="w-full max-w-2xl rounded-lg bg-white shadow-xl">
          <form @submit.prevent="handleSubmit" class="flex h-[90vh] flex-col">
            ...
            <div class="flex-grow space-y-6 overflow-y-auto p-6">
              <!-- Questions Section -->
              <div>
                <label class="block text-sm font-medium text-gray-700">Question(s)</label>
              ...
              <!-- Answers Section -->
              <div>
                <label class="block text-sm font-medium text-gray-700">Answer(s)</label>
            ...
            <div class="flex justify-end border-t p-4">
              <button type="submit" class="rounded-lg bg-green-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-green-700">
                {{ isEditing ? 'Update FAQ' : 'Submit FAQ' }}
              </button>
            </div>
          </form>
        </div>
...
const saveFaqsToLocalStorage = () => {
  localStorage.setItem('managedFaqs', JSON.stringify(faqs.value));
};

const loadFaqsFromLocalStorage = () => {
  const saved = localStorage.getItem('managedFaqs');
  if (saved) {
...
const openEditModal = (faq) => {
  ...
  newFaq.value = JSON.parse(JSON.stringify(faq)); // Deep copy to edit
  showModal.value = true;
};
...
const handleSubmit = () => {
  if (isEditing.value) {
    ...
    faqs.value.push({ ...newFaq.value, id: Date.now() });
  }
  saveFaqsToLocalStorage();
  closeModal();
};

// --- ACCORDION & DELETE LOGIC ---
const toggleFaq = (id) => {
